...
const displayImageList = imageList === undefined ? [] : imageList.filter(image => image.position === ('display'));
const orderedImageList = [];
var deletedDBImageList = [];
...
const fnAddImageToList = () => {
  $(document).on('change', '#product_images_area .image-input', function(){
    ...
    if(orderedImageList.length >= MAX_IMAGE_COUNT){
      $('#product_images_area .add-image-area').addClass('d-none');
    }
...
// DB 이미지 파일들 모두 출력
const fnPreviewDBImages = () => {
  if(displayImageList === []){
    return;
  }
  $.each(displayImageList, function(i, image){
    orderedImageList.push(image);
    fnPreviewImage(i, image.path + '/' + image.filesystemName, $('#product_images_area .add-image-area'));
  });
  if(displayImageList.length >= MAX_IMAGE_COUNT){
    $('#product_images_area .add-image-area').addClass('d-none');
  }
}
...
const fnDeleteImage = () => {
  $(document).on('click', '#product_images_area .btn_delete_image', function(ev){
    ...
    if(orderedImageList[index].filesystemName !== null){
      deletedDBImageList.push(orderedImageList[index].filesystemName);
    }
    orderedImageList.splice(index, 1);
    fnPreviewImageList();
    if(orderedImageList.length === MAX_IMAGE_COUNT - 1) {
      $('#product_images_area .add-image-area').removeClass('d-none');
    }
...
const fnCheckRequired = () => {
  $(document).on('click', '#btn_edit_product', function(){
    ...
    // 썸네일 이미지가 입력되었는지 체크
    fnToggleValid($('#thumbnail_area'), deleteThumbnail === true && $('#thumbnail')[0].files.length === 0, ' * 썸네일은 필수 입력 항목입니다.');
    ...
    $('#deletedDBImageList').val(deletedDBImageList);
